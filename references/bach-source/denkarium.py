...
bach denkarium read                 Letzte Einträge lesen
bach denkarium read --type logbuch  Nur Logbuch-Einträge
bach denkarium search "Begriff"     Suchen
bach denkarium brainstorm "Thema"   Ideen zu einem Thema sammeln
bach denkarium promote <id> task    Eintrag zu Task befördern
...
class DenkariumHandler(BaseHandler):
    """Handler für bach denkarium Operationen."""
    ...
    def get_operations(self) -> dict:
        return {
            ...
            "read": "Einträge lesen [--type=logbuch|denkarium] [--cat=CAT] [--limit=N]",
    ...
    def _read(self, args: list) -> tuple:
        ...
        if not rows:
            return True, "Keine Einträge gefunden."
    ...
    def _search(self, term: str) -> tuple:
        ...
        if not rows:
            return True, f"Keine Treffer für '{term}'."
    ...
    def _promote(self, entry_id: str, target: str, dry_run: bool) -> tuple:
        """Eintrag zu Task oder Wiki befördern."""
        try:
            eid = int(entry_id)
        except ValueError:
        ...
        if dry_run:
            conn.close()
            return True, f"[DRY-RUN] Würde #{eid} zu {target} befördern"
        ...
        if target == 'task':
            return True, f"[OK] Eintrag #{eid} zu Task #{promoted_id} befördert"
        else:
            return True, f"[OK] Eintrag #{eid} für Wiki-Erstellung markiert"
    ...
    def _categories(self) -> tuple:
        ...
        if not rows:
            return True, "Keine Einträge vorhanden."

        lines = ["DENKARIUM-KATEGORIEN", "=" * 40, ""]
        for row in rows:
            type_label = "Logbuch" if row['entry_type'] == 'logbuch' else 'Denkarium'
            lines.append(f"  {row['category']:20s} [{type_label:10s}] ({row['cnt']} Einträge)")
